...
for signature in ('void GameMode::activate()', 'void GameMode::onFrameStarted('):
    block = function('source/modes/GameMode.cpp', signature)
    assert re.search(r'rrSetCreaturesTextOverlay\(\*mGameMap,\s*getKeyboard\(\)->isModifierDown\(OIS::Keyboard::Alt\)\)', block)
