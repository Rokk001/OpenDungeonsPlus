...
std::string createCreatureDecayAnimation(Ogre::Entity* entity, const std::string& poseName, Ogre::Real duration)
{
    ...
    if(!skeleton->hasAnimation(name))
    {
        const auto* source = skeleton->getAnimation(poseName);
        auto* decay = skeleton->createAnimation(name, duration);
        for(unsigned short boneIndex = 0; boneIndex < skeleton->getNumBones(); ++boneIndex)
        {
            ...
            auto* track = decay->createNodeTrack(boneIndex);
            for(unsigned key = 0; key <= 2; ++key)
            {
                const Ogre::Real progress = key * 0.5f;
                auto* frame = track->createNodeKeyFrame(progress * duration);
...
void RenderManager::rrSetObjectAnimationState(MovableGameEntity* curAnimatedObject, const std::string& animation, bool loop)
{
    ...
    if(anim == EntityAnimation::rot_anim && dropCreature != nullptr)
    {
        ...
        auto* state = dropCreature->getAnimationState();
        if(state != nullptr)
        {
            state->setLoop(false);
...
void RenderManager::clearCreatureDecay(Creature* creature)
{
    ...
    auto* node = swarm->getParentSceneNode();
